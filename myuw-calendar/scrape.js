...
function scraper() {
    ...
    function parseSection(sec, cal, courses) {
        // TODO: holidays
        ...
        let rrule = {
            'freq':'WEEKLY',
            'until':(last_day.toLocaleString()),
            'interval':1,
            'byday':[]
        }
        for (let i = 0; i < 7; i++) {
            if (sec.meetings[0].meeting_days[days_long[i]] === true)
                rrule['byday'].push(days_short[i]);
        }
        cal.addEvent(subject, description, place, begin.toString(), end.toString(), rrule);
        //cal.addEvent(subject, description, place, begin.toString(), end.toString());
    }

    let cal = ics();
    let courses = WSData.current_course_data();
    courses.sections.forEach(section => {
        parseSection(section,cal,courses);
    });
