...
function scraper() {
    ...
    function parseHolidays(input, cal) {
        ...
        for (event of input) {
            ...
            if (event.myuw_categories['breaks'] === true) {
                let date = new Date(event.start+'T12:00:00-07:00');
                let end = new Date(event.end+'T12:00:00-07:00');
                cal.addEvent(event.summary,'holiday',"",date,end);
                while (date <= end) {
                    holidays.push(date);
                    date = new Date(date);
    ...
    let holidays = parseHolidays(WSData.current_academic_calendar_data(), cal);
